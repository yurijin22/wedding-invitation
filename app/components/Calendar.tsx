    ...
    <section className="py-24 px-8 bg-[#FAFAF7]">
      ...
      >
        <div className="text-center space-y-4">
          <p className="font-script text-3xl text-[#49311C]/70 italic">Save the date</p>
          <div className="section-divider" />
          <p className="text-xs tracking-[0.3em] text-[#b08a68] font-sans">
            {year} . {String(month + 1).padStart(2, "0")}
          </p>
        </div>

        <div className="bg-white rounded-2xl p-5 shadow-[0_2px_20px_rgba(73,49,28,0.06)]">
          {/* 요일 헤더 */}
            ...
            {DAYS_KO.map((d, i) => (
              <div
                ...
                className={`text-center text-[11px] font-sans font-light py-1 tracking-widest
                  ${i === 0 ? "text-[#c8826a]" : i === 6 ? "text-[#6a94b8]" : "text-[#b08a68]/60"}`}
              >
                {d}
              </div>
            ))}
                  ...
                  {day && (
                    <span
                      className={`
                        ...
                        transition-colors
                        ${day === weddingDay
                          ? "bg-[#49311C] text-white font-medium"
                          : di === 0
                          ? "text-[#c8826a]"
                          : di === 6
                          ? "text-[#6a94b8]"
        ...
        <div className="text-center space-y-1">
          <p className="text-sm tracking-widest text-[#49311C]">{wedding.time}</p>
          <p className="text-xs text-[#b08a68] font-sans font-light">
